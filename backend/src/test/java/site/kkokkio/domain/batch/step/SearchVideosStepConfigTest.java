...
@SpringBatchTest
@ActiveProfiles("test")
class SearchVideosStepConfigTest {

	private static final String STEP_NAME = BatchConstants.SEARCH_VIDEOS_STEP;
	...
	static class TestJobConfig {
		...
		@Bean
		public Job testJob(Step searchVideosStep) {
	...
	@Test
	@DisplayName("searchVideosStep 성공")
	void testSuccess() throws Exception {
		// given
		when(sourceService.searchYoutube()).thenReturn(new SearchStatsDto(5, 1));
		...
		assertThat(stepCtx.getInt(ExecutionContextKeys.SC_VIDEO_FETCHED)).isEqualTo(5);
		assertThat(stepCtx.getInt(ExecutionContextKeys.SC_VIDEO_API_FAIL)).isEqualTo(1);
	...
	@Test
	@DisplayName("searchVideosStep 성공: 빈 리스트 반환")
	void testEmptyList() throws Exception {
		// given
		when(sourceService.searchYoutube()).thenReturn(new SearchStatsDto(0, 0));
		...
		StepExecution stepExec = exec.getStepExecutions().iterator().next();
		ExecutionContext stepCtx = stepExec.getExecutionContext();
		assertThat(stepCtx.getInt(ExecutionContextKeys.SC_VIDEO_FETCHED)).isZero();
		assertThat(stepCtx.getInt(ExecutionContextKeys.SC_VIDEO_API_FAIL)).isZero();
	...
	@Test
	@DisplayName("searchVideosStep 실패: Service 예외 발생")
	void testFailure() throws Exception {
		...
		assertThat(jobCtx.containsKey(ExecutionContextKeys.SC_VIDEO_FETCHED)).isFalse();
		assertThat(jobCtx.containsKey(ExecutionContextKeys.SC_VIDEO_API_FAIL)).isFalse();
	...
	@Test
	@DisplayName("searchVideosStep 재시도 성공(allowStartIfComplete=true): 2회 연속 실행 시도 가능")
	void testAllowStartIfComplete() throws Exception {
		...
		verify(sourceService, times(2)).searchYoutube();
	}
}